...
If optional PREFIX is non-nil, unstow the files."
  (interactive "P\ni")
  (let* ((base-dir (rc/expand "../dots/"))
         (targets (directory-files (rc/expand dir base-dir) t)))
    (dolist (target (cdr (cdr targets)))
      (if (or (file-regular-p target)
              (cl-dolist (parent '(".config/"))
                (let ((parent (rc/expand parent base-dir)))
                  (when (and (file-in-directory-p target parent)
                             (not (file-equal-p target parent)))
                    (cl-return target)))))
          (let* ((target-name (rc/expand target base-dir))
                 (rel-name (file-relative-name target-name base-dir))
